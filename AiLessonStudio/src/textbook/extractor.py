...
class TextbookExtractor:
    """Advanced textbook extractor for various file formats"""

    def __init__(self, config):
        self.config = config

    def extract(self, file_path):
        """Extract content from textbook file"""
        # Delegate to ContentExtractor for backward compatibility
...
class ContentExtractor:
    ...
    def _generate_structure_recommendations(self, structure: Dict,
        ...
        # Note: structure['cloud_term_density'] doesn't exist - it's in quality dict
        # This is a bug in the original code, but we'll leave it as is for compatibility
        if metrics.get('cloud_term_density', 0) < 0.05:
